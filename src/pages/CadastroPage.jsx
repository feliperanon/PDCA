...
import { Users, Truck, Settings, Hammer, Building2, X, Plus, Trash2, Search, Edit, Plane, Stethoscope, CheckCircle, UserMinus } from 'lucide-react';

export function CadastroPage() {
    const [selectedCategory, setSelectedCategory] = useState('employees'); // 'employees' | 'machines' etc
    ...
    const handleIconClick = (id) => {
        ...
        setIsModalOpen(true); // Open "Floating Page" for registration
    };
    ...
    return (
        <div className="page" style={{ maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <div style={{ marginTop: '20px' }}>
                <h2 style={{ fontSize: '18px', marginBottom: '15px', color: '#334155', borderBottom: '1px solid #e2e8f0', paddingBottom: '10px' }}>
                    Listagem: {activeCat.title}
                </h2>
                {/* Pass handleEdit to list so it can trigger modal with data */}
                <activeCat.listComp
                    onEdit={(item) => {
                        setEditData(item);
...
function EmployeeForm({ onClose, initialData }) {
    const [formData, setFormData] = useState({
        ...
        status: 'ativo' // ativo, afastado, ferias
    });
...
function EmployeeList({ onEdit }) {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                for (let i = 0; i < Math.min(rawData.length, 10); i++) {
                    const row = rawData[i];
                    // Check if row has "Colaborador" or "Matrícula" or "Nome"
                ...
                if (headerRowIndex === -1) {
                    return alert("Não foi possível encontrar o cabeçalho (Colunas 'Colaborador', 'Nome' ou 'Matrícula') nas primeiras 10 linhas.");
                }
                ...
                if (!window.confirm(`Encontrados ${dataRows.length} registros (Cabeçalho na linha ${headerRowIndex + 1}). Importar?`)) return;
                ...
                for (let rowArray of dataRows) {
                    ...
                    const nome = normalizedRow['colaborador'] || normalizedRow['nome'] || normalizedRow['name'] || normalizedRow['funcionario'];

                    if (!nome) continue;

                    const matricula = String(normalizedRow['matricula'] || normalizedRow['id'] || "");
                    const centroCusto = normalizedRow['centro de custo'] || normalizedRow['cc'] || "";
                    const funcao = normalizedRow['cargo'] || normalizedRow['funcao'] || "";
                    // Helper to parse dates
                    const parseDate = (val) => {
                        if (!val) return "";
                        if (val instanceof Date) return val.toISOString().split('T')[0];
                        // Excel Serial Date (numbers like 44106)
                    ...
                    const admissao = parseDate(normalizedRow['data admissao'] || normalizedRow['admissao']);
                    const nascimento = parseDate(normalizedRow['aniversario'] || normalizedRow['nascimento']);
                ...
                if (importedCount > 0) {
                    fetchEmployees();
                    alert(`Sucesso! ${importedCount} colaboradores importados.`);
                } else {
    ...
    // Helper to clean Centro de Custo
    function formatCC(cc) {
        if (!cc) return "-";
        // Remove "EXPEDICAO " or "EXPEDIÇÃO " (case insensitive)
        return cc.replace(/EXPEDI[CÇ][AÃ]O\s*/i, "").trim() || cc;
    }

    const filtered = employees.filter(e => e.nome?.toLowerCase().includes(filter.toLowerCase()) || e.matricula?.includes(filter));

    return (
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
            <div style={{ padding: '15px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', gap: '20px' }}>
            ...
            {filtered.length === 0 && <div style={{ padding: '30px', textAlign: 'center', color: '#94a3b8' }}>Nenhum colaborador encontrado.</div>}
