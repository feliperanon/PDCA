...
// Página simples de dashboard de PDCA, usando dados fictícios por enquanto.
...
// =========================
//
// Depois a gente troca isso pelo Firestore.
// A ideia é: você ter algo na tela já funcionando.
...
const MOCK_PDCAS = [
  {
    ...
    criadoEm: "2025-11-30T22:51:03", // data em texto ISO
  },
  {
    id: "PDCA-2025-6935",
...
// ====================================================
// 2) COMPONENTE PRINCIPAL – DASHBOARD DE PDCA
// ====================================================

export function PdcaDashboardPage() {
  // estado com a lista de PDCAs (por enquanto, mock)
  const [pdcas] = useState(MOCK_PDCAS);

  // qual visão salva está ativa
  const [activeViewId, setActiveViewId] = useState(null);

  // termo de busca
  const [searchTerm, setSearchTerm] = useState("");

  // Definição das visões salvas (filtros rápidos)
  const savedViews = [
    {
      id: "criticos-hoje",
  ...
  const filteredPdcas = useMemo(() => {
    ...
    // Filtro de texto (código ou título)
    if (searchTerm.trim() !== "") {
    ...
    return list;
  }, [pdcas, activeViewId, searchTerm, savedViews]);
      ...
      <section className="border border-gray-200 rounded-md divide-y divide-gray-200 bg-white">
        ...
        {filteredPdcas.map((pdca) => {
          ...
          return (
            <div
              key={pdca.id}
              className="w-full text-left p-4 hover:bg-gray-50 flex flex-col gap-1 cursor-default"
            >
...
// ====================================================
// 3) HELPERS – FUNÇÕES AUXILIARES (FORA DO COMPONENTE)
// ====================================================

// Converte string de data (tipo "2025-11-30T22:51:03") em objeto Date
function firestoreStringDateToDate(str) {
  if (!str) return null;
  return new Date(str);
}

// PDCAs criados nos últimos X dias
...
// PDCAs com status = "Padronizado" no mês atual
...
// PDCAs que voltaram para Plan (aqui uso um campo fictício)
function getPdcasBackToPlan(pdcas) {
  // Por enquanto, nenhum mock tem voltouParaPlan = true, então vai ficar vazio.
  // Quando você começar a registrar isso de verdade, é só gravar esse campo no PDCA.
  return pdcas.filter(
    (p) => p.status === "Planejando" && p.voltouParaPlan === true
  );
}

// Componente de bloco da timeline
