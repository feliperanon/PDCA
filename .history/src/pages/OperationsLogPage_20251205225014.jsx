...
export function OperationsLogPage() {
    ...
    const styles = `
        ...
        .btn-pdca-manual { background: white; color: #333; border: 1px solid #ccc; padding: 10px; border-radius: 6px; width: 100%; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; margin-top: 10px; }
        .btn-pdca-manual:hover { background: #f9fafb; }
        ...
        .badge-Quebra { background: #7c3aed; } /* Roxo */
        .badge-Ruptura { background: #db2777; } /* Rosa */
        .badge-Cliente { background: #ea580c; } /* Laranja */
        .badge-Treinamento { background: #0891b2; } /* Ciano */
        .badge-Indicador { background: #059669; } /* Verde Escuro */
        .badge-Planejamento { background: #4f46e5; } /* Índigo */
        .badge-Evento { background: #64748b; } /* Cinza Azulado */
        .badge-Outros { background: #9ca3af; } /* Cinza Default */
        ...
        /* Select dentro do Modal de Edição */
    ...
    const analisarTexto = (texto) => {
        ...
        // Tipos (Prioridade de palavras-chave)
    ...
    const abrirModalPDCA = (origem) => {
        ...
        setPdcaForm({
            logId: (origem && origem.id) ? origem.id : null,
            descricao: textoBase,
    ...
    const confirmarSalvarPDCA = () => {
        const novoPDCA = { ...pdcaForm, id: Date.now() + Math.random(), dataCriacao: new Date().toLocaleDateString() };
        setPdcas([...pdcas, novoPDCA]);
        setShowPdcaModal(false);
        alert('Plano de Ação salvo!');
    };

    // --- TESTE SEM ERRO DE KEY ---
    const gerarDadosOntem = () => {
        const ontem = new Date();
        ontem.setDate(ontem.getDate() - 1);
        const dataOntem = ontem.toLocaleDateString();
        // Usa Random para evitar chaves duplicadas
        const logsTeste = [{ id: Date.now() + Math.random(), data: dataOntem, hora: '14:30:00', tipo: 'Erro / Falha operacional', categoria: 'RH', cliente: 'Geral', textoOriginal: 'Falta de pessoal' }];
    ...
    return (
        <div className="app-root">
            ...
            <div className="main-grid">
                {/* LADO ESQUERDO: INPUT (Sem Editar) */}
                <div className="smart-input-container">
                    ...
                    <textarea 
                        ...
                        placeholder="Descreva o fato... (Ex: Treinamento de segurança realizado com a equipe)"
                        value={inputTexto}
                ...
                <div className="intelligence-card">
                    ...
                    {/* BOTÃO PDCA MANUAL SEMPRE VISÍVEL */}
                    <button className="btn-pdca-manual" onClick={() => abrirModalPDCA('manual')}>
                        <IconPlus /> Novo PDCA Manual
                    </button>
                ...
                <div className="modal-overlay">
                    <section className="modal-card">
                        ...
                        <div className="card-content">
                            ...
                            <div>
                                <label className="modal-label">Classificação (Tipo)</label>
                                <select 
            ...
            {showPdcaModal && (
                <div className="modal-overlay">
                    <section className="modal-card plan-card">
                        ...
                        <div className="card-content">
                            <div>
                                <label className="modal-label">Descrição do Problema</label>
                                <textarea className="modal-textarea" rows="3" value={pdcaForm.descricao} onChange={(e) => setPdcaForm({...pdcaForm, descricao: e.target.value})} />
                            </div>
                            <div>
                                <label className="modal-label">Causas (Sugestão IA)</label>
                                <textarea className="modal-textarea" rows="3" value={pdcaForm.causas} onChange={(e) => setPdcaForm({...pdcaForm, causas: e.target.value})} />
                            </div>
                            <div className="form-grid-2">
                                <div><label className="modal-label">Indicador Antes</label><input className="modal-textarea" style={{minHeight:'40px'}} placeholder="Ref..." value={pdcaForm.indicadorAntes} onChange={(e) => setPdcaForm({...pdcaForm, indicadorAntes: e.target.value})} /></div>
                                <div><label className="modal-label">Indicador Meta</label><input className="modal-textarea" style={{minHeight:'40px'}} placeholder="Alvo..." value={pdcaForm.indicadorMeta} onChange={(e) => setPdcaForm({...pdcaForm, indicadorMeta: e.target.value})} /></div>
                            </div>
                            <div>
                                <label className="modal-label">Plano de Ação (Sugestão IA)</label>
                                <textarea className="modal-textarea" rows="4" value={pdcaForm.planoAcao} onChange={(e) => setPdcaForm({...pdcaForm, planoAcao: e.target.value})} />
                            </div>
                            <div className="section-actions">
                                <button className="btn-secondary" onClick={() => setShowPdcaModal(false)}>Cancelar</button>
                                <button className="btn-primary" onClick={confirmarSalvarPDCA}>Salvar Plan</button>
                            </div>
                        </div>
                    </section>
                </div>
