...
export function OperationsDashboard() {
    // --- 1. ESTILOS EMBUTIDOS (Para garantir que o design funciona) ---
    // Copiei o seu CSS Pro e compactei aqui para não depender de arquivos externos
    const styles = `
    ...
    // Função para pegar a data de HOJE em formato universal (YYYY-MM-DD)
    // Isso evita o erro de "formato de data diferente"
    ...
    // Estado da Saúde do Dia (Recalculado a cada mudança)
    ...
    useEffect(() => {
        // 1. Salvar no navegador
        localStorage.setItem('diario_full', JSON.stringify(registros));
        
        // 2. Recalcular a Saúde do Dia
        calcularSaude();
    }, [registros]);

    const calcularSaude = () => {
        const hojeISO = getHojeISO();
        // Filtra APENAS registros que tenham a data ISO de hoje
        ...
        if (pontos < 80) { cor = '#eab308'; label = 'Atenção'; } // Amarelo
        if (pontos < 60) { cor = '#dc2626'; label = 'Crítico'; } // Vermelho

        setSaude({ score: pontos, label, cor });
    };

    // Conta erros de hoje por categoria para os cartões
    ...
    const salvarRegistro = () => {
        ...
        const novo = {
            id: Date.now(),
            dataISO: getHojeISO(), // Data universal para lógica
            dataVisivel: getHojeFormatado(), // Data para exibir
            hora: getHoraAtual(),
            ...form
        };
    ...
    // Verifica se TEM erros hoje para mostrar o alerta vermelho
    const temErroHoje = registros.some(r => r.dataISO === getHojeISO() && r.tipo === 'Erro');

    return (
        <div className="app-root">
            <style>{styles}</style> {/* Injeta o CSS aqui mesmo */}

            {/* HEADER */}
            <div className="header">
                <div>
                    <h1 style={{margin:0, color: '#1f2937'}}>Painel Operacional</h1>
                    <p style={{margin:0, color: '#6b7280'}}>Data: {getHojeFormatado()}</p>
                    <button onClick={resetar} style={{background:'none', border:'none', textDecoration:'underline', cursor:'pointer', fontSize:'11px', color:'#888'}}>Resetar Sistema</button>
                </div>
                ...
                <div className="modal-overlay">
                    <div className="modal-content">
                        <div style={{display:'flex', justifyContent:'space-between', marginBottom:'20px'}}>
                            <h2 style={{margin:0}}>PDCA Rápido</h2>
                            <button onClick={() => setModalAberto(false)} style={{background:'none', border:'none', fontSize:'20px', cursor:'pointer'}}>×</button>
                        </div>
                        
                        <label><b>P</b> (Problema - Automático)</label>
                        <input value={pdca.problema} readOnly style={{background:'#f3f4f6'}} />
                        
                        <label><b>D</b> (Causa Raiz)</label>
                        <textarea placeholder="Por que aconteceu?" value={pdca.causa} onChange={e => setPdca({...pdca, causa: e.target.value})} />
                        
                        <label><b>A</b> (Ação Imediata)</label>
                        <textarea placeholder="O que será feito?" value={pdca.acao} onChange={e => setPdca({...pdca, acao: e.target.value})} />

                        <button className="btn" onClick={() => { alert('PDCA Salvo!'); setModalAberto(false); }}>Confirmar Plano</button>
                    </div>
                </div>
            )}
        </div>
