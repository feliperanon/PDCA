...
export function HomePage() {
  ...
  const totalPlan = pdcas.filter(
    (p) => getStageFromStatus(p.status) === "Plan"
  ).length;
  const totalDo = pdcas.filter(
    (p) => getStageFromStatus(p.status) === "Do"
  ).length;
  const totalCheck = pdcas.filter(
    (p) => getStageFromStatus(p.status) === "Check"
  ).length;
  const totalAct = pdcas.filter(
    (p) => getStageFromStatus(p.status) === "Act"
  ).length;

  const totalAtivos = pdcas.filter(
    (p) => p.status !== "Padronizado"
  ).length;
