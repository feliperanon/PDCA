...
export function OperationsLogPage() {
    ...
    const styles = `
        ...
        /* Morning Call */
        .morning-alert { background: #fff7ed; border: 1px solid #fed7aa; color: #9a3412; padding: 15px; border-radius: 8px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; animation: fadeIn 0.5s; box-shadow: 0 2px 5px rgba(251, 146, 60, 0.1); }
        ...
        .type-selector { margin-top: 10px; width: 100%; padding: 8px; border-radius: 6px; border: 1px solid #cbd5e1; font-size: 13px; background: #f8fafc; }
        ...
        .badge-Cliente { background: #9333ea; }
        .badge-Outros { background: var(--primary); }
    ...
    const [selectedType, setSelectedType] = useState(TIPOS_OCORRENCIA[0]); // Para edição/ajuste manual
    ...
    const analisarTexto = (texto) => {
        ...
        // Categorias Extensivas
        if (t.includes('rh') || t.includes('falt') || t.includes('atestad') || t.includes('ferias') || t.includes('equipe')) cat = 'RH';
        else if (t.includes('frota') || t.includes('caminhao') || t.includes('pneu') || t.includes('veiculo') || t.includes('multa')) cat = 'Frota';
        else if (t.includes('qualidade') || t.includes('avaria') || t.includes('devolucao') || t.includes('cliente') || t.includes('reclam')) cat = 'Qualidade';
        else if (t.includes('juridico') || t.includes('processo') || t.includes('advogado')) cat = 'Jurídico';
        else if (t.includes('logistica') || t.includes('expedicao') || t.includes('recebimento') || t.includes('estoque')) cat = 'Logística';
        else if (t.includes('manutencao') || t.includes('obra') || t.includes('predial')) cat = 'Manutenção';
        else if (t.includes('ti') || t.includes('sistema') || t.includes('internet') || t.includes('computador')) cat = 'TI';

        // Tipos (Baseado na lista do usuário)
        if (t.includes('melhoria') || t.includes('ideia') || t.includes('oportunidade')) tipo = 'Melhoria / Oportunidade';
        else if (t.includes('alerta') || t.includes('risco') || t.includes('atraso') || t.includes('quase')) tipo = 'Alerta / Risco';
        else if (t.includes('quebra') || t.includes('dano') || t.includes('perda') || t.includes('lixo')) tipo = 'Quebra / Perda';
        else if (t.includes('ruptura') || t.includes('indisponivel') || t.includes('acabou')) tipo = 'Ruptura / Disponibilidade';
        else if (t.includes('cliente') || t.includes('reclamacao') || t.includes('sac')) tipo = 'Cliente / Reclamação';
        else if (t.includes('treinamento') || t.includes('curso') || t.includes('capacita')) tipo = 'Treinamento / Capacitação';
        else if (t.includes('resultado') || t.includes('recorde') || t.includes('meta')) tipo = 'Indicador positivo / Resultado';
        else if (t.includes('planejamento') || t.includes('decisao') || t.includes('prioridade')) tipo = 'Planejamento / Decisão do dia';
        else if (t.includes('chuva') || t.includes('transito') || t.includes('greve') || t.includes('luz')) tipo = 'Evento externo / Fator externo';
    ...
    const salvarRegistro = () => {
        ...
        // Se estiver editando, usa o tipo selecionado manualmente, senão usa o sugerido
        ...
        if (editandoId) {
            ...
            setEditandoId(null);
            setSelectedType(TIPOS_OCORRENCIA[0]);
        } else {
            const novoLog = {
                id: Date.now(),
    ...
    // --- FUNÇÃO DE TESTE PARA VOCÊ VER O "MORNING CALL" AGORA ---
    const gerarDadosOntem = () => {
        const ontem = new Date();
        ontem.setDate(ontem.getDate() - 1);
        const dataOntem = ontem.toLocaleDateString();
        const logsTeste = [
            { id: 998, data: dataOntem, hora: '08:00:00', tipo: 'Erro / Falha operacional', categoria: 'Logística', cliente: 'Geral', textoOriginal: 'Falha no carregamento noturno' },
            { id: 999, data: dataOntem, hora: '14:30:00', tipo: 'Erro / Falha operacional', categoria: 'RH', cliente: 'Geral', textoOriginal: 'Falta de pessoal na doca' }
        ];
        setLogs([...logs, ...logsTeste]);
        alert("Dados de ontem gerados! O alerta aparecerá no topo.");
    };

    // --- AÇÕES TIMELINE ---
    const editarLog = (log) => {
        setInputTexto(log.textoOriginal);
        setSelectedType(log.tipo); // Carrega o tipo atual no select
    ...
    const excluirLog = (id) => {
        if(window.confirm('Excluir este registro permanentemente?')) {
            ...
            if (editandoId === id) { setEditandoId(null); setInputTexto(''); }
        }
    };

    // --- PDCA COM SUGESTÕES DE IA ---
    const gerarSugestaoPDCA = (texto) => {
        const t = texto.toLowerCase();
        let sugestao = { causas: '', plano: '' };

        if (t.includes('falta') || t.includes('rh')) {
            sugestao.causas = "1. Ausência não programada.\n2. Falta de banco de talentos reserva.\n3. Clima organizacional.";
            sugestao.plano = "Acionar lista de extras. Iniciar processo seletivo urgente.";
        } else if (t.includes('quebra') || t.includes('frota')) {
            sugestao.causas = "1. Desgaste natural.\n2. Falta de preventiva.\n3. Má condução.";
            sugestao.plano = "Enviar para oficina. Revisar plano de manutenção da frota.";
        } else if (t.includes('atraso')) {
            sugestao.causas = "1. Trânsito intenso.\n2. Demora no carregamento.\n3. Rota mal planejada.";
            sugestao.plano = "Revisar roteirização. Ajustar janela de entrega.";
        } else {
            sugestao.causas = "1. Processo indefinido.\n2. Falha humana.\n3. Recurso insuficiente.";
            sugestao.plano = "Investigar causa raiz e treinar a equipe.";
        }
        return sugestao;
    };
    ...
    const abrirModalPDCA = (origem) => {
        ...
        const sugestoes = gerarSugestaoPDCA(textoBase);

        setPdcaForm({
            logId: origem.id || null,
            descricao: textoBase,
            causas: sugestoes.causas, // Preenchimento Automático
            indicadorAntes: '',
            indicadorMeta: '',
            metaDescritiva: 'Eliminar a causa raiz.',
            planoAcao: sugestoes.plano // Preenchimento Automático
        });
        setShowModal(true);
    };
    ...
    // KPI Dinâmico: Cria um objeto contando erros por categoria existente HOJE
    ...
    const getBadgeClass = (tipo) => {
        if(tipo.includes('Erro') || tipo.includes('Falha') || tipo.includes('Quebra') || tipo.includes('Ruptura')) return 'badge-Erro';
        if(tipo.includes('Alerta') || tipo.includes('Risco')) return 'badge-Alerta';
        if(tipo.includes('Melhoria') || tipo.includes('Positivo')) return 'badge-Melhoria';
        ...
        <div className="app-root">
            ...
            <div className="header">
                <div>
                    ...
                    {/* Botão Secreto para Testar o Morning Call */}
                    <button onClick={gerarDadosOntem} style={{marginLeft:'10px', fontSize:'10px', cursor:'pointer', border:'1px dashed #ccc', background:'none'}}>Gerar Dados Ontem (Teste)</button>
                </div>
            ...
            {/* MORNING CALL (Só aparece se tiver erro ontem) */}
            {alertaOntem && (
            ...
            {/* KPIs DINÂMICOS - Só mostra o que tem erro hoje */}
            <div className="kpi-grid">
                {Object.keys(statsHoje).length === 0 ? (
                    <div style={{width:'100%', textAlign:'center', color:'#999', fontSize:'13px', padding:'10px', border:'1px dashed #eee', borderRadius:'8px'}}>
                        Sem erros operacionais registrados hoje.
                    </div>
                ) : (
                    Object.entries(statsHoje).map(([cat, qtd]) => (
                        <div className="kpi-card" key={cat}>
                            <div className="kpi-indicator" style={{background: '#dc2626'}}></div>
                            <h3>{cat}</h3>
                            <span className="kpi-value">{qtd} <span style={{fontSize:'12px', fontWeight:'400', color:'#888'}}>Erros</span></span>
                        </div>
                    ))
                )}
            </div>
            ...
            <div className="main-grid">
                ...
                <div className="smart-input-container">
                    ...
                    {/* SELETOR DE TIPO (Só aparece se estiver editando ou para ajuste fino) */}
                    <select 
                    ...
                    <div style={{fontSize:'11px', color:'#9ca3af', marginTop:'10px', textAlign:'center'}}>
                        A IA detecta a Categoria. Você pode ajustar o Tipo acima.
                    </div>
                </div>
            ...
            {/* MODAL PDCA (Layout Aprovado + Sugestões IA) */}
            {showModal && (
                <div className="modal-overlay">
                    <section className="detail-card plan-card">
                        ...
                        <div className="card-content">
                            ...
                            <label>Causas (Hipóteses - Sugestão Automática)</label>
                            ...
                            <label>Plano de Ação (Sugestão Automática)</label>
