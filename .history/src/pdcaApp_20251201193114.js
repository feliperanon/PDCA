...
export function initPdcaApp() {
  ...
  async function carregarDados(uid) {
    ...
    atualizarDashboardResponsaveis(mapaResponsavel, tabelaResponsaveis, resumoResponsaveis);
  }
  ...
  function atualizarTabelaPrioridades(
    tabela,
    mapaArea,
    filtro = "",
    resumoTopAreas
    ...
    if (areas.length === 0) {
      tabela.innerHTML = "<tr><td>Não há PDCAs em aberto.</td></tr>";
      if (resumoTopAreas) {
    ...
    if (resumoTopAreas) {
      ...
      const partes = top.map((l) => `${l.area}: ${l.total} PDCA(s)`);
  ...
  function atualizarDashboardResponsaveis(
    mapaResponsavel,
    tabelaResponsaveis,
    resumoResponsaveis
  ) {
    const nomes = Object.keys(mapaResponsavel);

    if (!tabelaResponsaveis || !resumoResponsaveis) return;

    if (nomes.length === 0) {
      tabelaResponsaveis.innerHTML =
        "<tr><td>Não há PDCAs em aberto por responsável.</td></tr>";
      resumoResponsaveis.textContent = "Nenhum PDCA em aberto no momento.";
      return;
    }

    const linhas = nomes.map((nome) => {
      const info = mapaResponsavel[nome];
      return {
        responsavel: nome,
        abertos: info.abertos,
        atrasados: info.atrasados,
      };
    });

    linhas.sort((a, b) => b.abertos - a.abertos);
    ...
    html += "</tbody>";
    tabelaResponsaveis.innerHTML = html;

    const top = linhas.slice(0, 3);
    const partes = top.map(
      (l) =>
        `${l.responsavel}: ${l.abertos} aberto(s), ${l.atrasados} atrasado(s)`
    );

    resumoResponsaveis.textContent =
      "Top responsáveis com mais PDCAs abertos — " + partes.join(" | ");
  }
  ...
  listaPdca.addEventListener("click", async (event) => {
    ...
    try {
      ...
      pdcaDetalhe.innerHTML = `
        ...
        <p><strong>Área:</strong> ${d.plan?.area || ""}</p>
        <p><strong>Responsável:</strong> ${d.plan?.responsavel || "-"}</p>
        <p><strong>Time / turno:</strong> ${d.plan?.timeOuTurno || "-"}</p>
        <p><strong>Data alvo:</strong> ${dataAlvoTexto}</p>
        <p><strong>Prioridade:</strong> ${prioridadeBadge}</p>
        <p><strong>Categoria:</strong> ${d.plan?.categoria || ""}</p>
        ...
        <hr>
        <h3>P – Plan</h3>
        <p><strong>Tipo de objeto:</strong> ${d.plan?.tipoObjeto || "-"}</p>
        <p><strong>Descrição do objeto:</strong> ${
          d.plan?.descricaoObjeto || "-"
        }</p>
        <p><strong>Problema:</strong> ${d.plan?.problema || ""}</p>
