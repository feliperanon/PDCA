...
window.addEventListener("DOMContentLoaded", () => {
  ...
  selectCheckPdca.addEventListener("change", async () => {
    ...
    try {
      ...
      checkResumo.innerHTML = `
        ...
        <p><strong>Problema:</strong> ${d.plan?.problema || ""}</p>
        <p><strong>Plano de ação:</strong> ${d.plan?.planoAcao || ""}</p>
        <p><strong>Ações realizadas (Do):</strong> ${d.do?.acoesRealizadas || ""}</p>
      `;
    } catch (e) {
  ...
  selectActPdca.addEventListener("change", async () => {
    ...
    try {
      ...
      actResumo.innerHTML = `
        ...
        <p><strong>Indicadores antes:</strong> ${d.check?.indicadoresAntes || ""}</p>
        <p><strong>Indicadores depois:</strong> ${d.check?.indicadoresDepois || ""}</p>
      `;
    } catch (e) {
  ...
  formAct.onsubmit = async (e) => {
    ...
    // Se funcionou: vira padrão (status = Padronizado)
    // Se não funcionou: volta para Planejando (ajustar o Plan e recomeçar ciclo)
